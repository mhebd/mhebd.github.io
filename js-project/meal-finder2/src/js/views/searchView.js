...
export const clearResCont = () => {
  elements.searchResList.innerHTML = '';
  elements.resPages.innerHTML = '';
};

export const highlitedSelected = id => {
  const arrResult = [...document.querySelectorAll('.results__link')];
  arrResult.forEach(el => el.classList.remove('results__link--active'));
...
const createButton = (page, type) => {
  const button = `
    <button class="btn-inline results__btn--${type === 'next' ? 'next' : 'prev'}" data-togo=${type === 'next' ? page + 1 : page - 1}>
...
const renderButton = (page, numRess, resPerPage) => {
  const pages = Math.ceil(numRess / resPerPage);
